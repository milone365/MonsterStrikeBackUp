...
public class CharacterBox : MonoBehaviour
{
    ...
    public bool getSSActive()
    {
        return ss_active;
    }
    public SS_Count ssCountObj;
    bool running;
    public void SetBox(Monster monster)
    {
        this.monster = monster;
        this.monster.box = this;
    ...
    void ActiveSS()
    {
        Monster current = PlayerController.instance.current_monster;
        if (current != monster) return;
        running = PlayerController.instance.getIsRunning();
        if (running) return;
        ...
        if(canActiveSS)
        {
            ...
            monster.ShowSSEffect(ss_active);
        }
    }
