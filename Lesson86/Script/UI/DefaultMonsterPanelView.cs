...
public class DefaultMonsterPanelView : MonoBehaviour
{
    ...
    public void SetTeam(MonsterData []list)
    {
        team = GetComponentsInChildren<MonsterViewBox>();
        int hpcount = 0;
        for (int i=0;i<team.Length;i++)
        {
            MonsterData d = list[i];
            if (d == null) continue;
            team[i].Init(d, ViewBoxType.Menu, i);
            hpcount += Helper.ByLevel(d.Level, d.hp);
        }
